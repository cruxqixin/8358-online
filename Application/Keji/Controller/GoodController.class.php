...
class GoodController extends BaseController{
	public function index(){
		...
		if($_POST["keyword"]&&$_POST['keyword']!='è¯·è¾“å…¥å…³é”®å­—'){
		...
		$where['STATUS']=0;
		$countt=$goodkindd->where($where)->count();//È«²¿
			...
			if($_POST['ID']==''){
				...
				$PRO_KNAMES= $_POST["kindid"];
					...
					if ($row){ 
						$search=new SearchModel('Search');
						 ...
						 $search->add($data);
						$this->success('æ·»åŠ æˆåŠŸ',U('Good/index'));
						return;
					}else { 
						$this->success('æ·»åŠ æˆåŠŸ',U('Good/index'));
						return;
				...
				if($save){
					 ...
					 $search->where('ITEMID='.$da["ID"])->save($datae);
						$this->success('ä¿®æ”¹æˆåŠŸ',U('Good/index'));
						return;
					}else {
						$this->success('ä¿®æ”¹å¤±è´¥',U('Good/index'));
						return;
					}
			}
